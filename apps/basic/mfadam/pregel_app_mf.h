...
// optimization: Adam, faster descent and larger oscillation than SGD
// but using SGD can reach a smaller loss than using Adam in large data.
...
class K_Dim {
 ...
 public:
  ...
  void init_zero(const size_t n) {
    for (size_t i = 0; i < n; i++) {
      _vec.emplace_back(0.0);
    }
  }

  void init_random(const size_t n) {
    for (size_t i = 0; i < n; i++) {
...
class MFVertex_pregel : public Vertex<VertexID, MFValue_pregel, MFMsg_pregel> {
 public:
  ...
  void adam(K_Dim& grad) {
    value().m = value().m * BETA1 + grad * (1 - BETA1);
    value().v = value().v * BETA2 + grad * grad * (1 - BETA2);
    auto m_b = value().m / (1 - BETA1);
    auto v_b = value().v / (1 - BETA2);
    value().dim -= (m_b * ALPHA) / (v_b.sqrt() - EPSILON);
  }
      ...
      if (fabs(agg->last_round - agg->last_last_round) < LOSS_DIF_HRESHOLD ||
        ...
        if (value().label == 0) {  // user
          ...
          for (size_t i = 0; i < messages.size(); i++) {
            ...
            auto grad = (q_dim * val + p_dim * LAMBDA) * 2;
            adam(grad);
            loss += val * val;
          }
        } else {  // item
...
class MFWorker_pregel : public Worker<MFVertex_pregel, MFAgg_pregel> {
 ...
 public:
  ...
  virtual MFVertex_pregel* toVertex(char* line) {
    ...
    int id = atoi(pch);
    v->id = id;
    v->value().dim.init_random(FEATURE_NUM);
    v->value().m.init_zero(FEATURE_NUM);
    v->value().v.init_zero(FEATURE_NUM);
    pch = strtok(NULL, "\t");
    int label = atoi(pch);
    v->value().label = label;
    pch = strtok(NULL, "\t");
