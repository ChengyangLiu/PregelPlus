...
template <class RVertexT,
          class AggregatorT = DummyAgg>  // user-defined RVertexT
class RWorker {
       ...
       it++)
                    {
                            ...
                            for(int i=0; i<_num_workers; i++)
                            {
                                    ...
                                    if(set.count(v->id))
                                    {
                                            message_buffer->add_respond_implicit(v->id,
       i, respond);
                                    }
                            }
                    }
  ...
  void run(const WorkerParams& params) {
    ...
    while (true) {
      ...
      if (hasRR) {
        ...
        respond_reqs();
        step_resp_num = all_sum_LL(
            message_buffer
                ->get_total_resp());  // all_sum_LL instead of master_sum_LL
    ...
    if (_my_rank == MASTER_RANK)
      cout << "Total #msgs=" << global_msg_num
           ...
           << ", Total #reqs=" << global_req_num
           << ", Total #resps=" << global_resp_num << endl;
