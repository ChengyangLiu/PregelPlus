 ...
 public:
  ...
  /*
              void active_compute()
              {
                      ...
                      for(VertexIter it=vertexes.begin(); it!=vertexes.end();
     it++)
                      {
                              KeyT vid=(*it)->id;
                              ...
                              if(mit->second->size()==0)
                              {
                                      if((*it)->is_active())
                                      {
                                              (*it)->compute(empty);
                                              AggregatorT*
     agg=(AggregatorT*)get_aggregator();
                                              if(agg!=NULL)
     agg->stepPartial(*it);
                                              if((*it)->is_active())
     active_count++;
...
#else
  void load_graph(const char* inpath)
  // filename.adj and filename.partn are needed in given path.
  // n is the given partition number.
  {
    string str = inpath;
  ...
  void run_report(const WorkerParams& params, const string reportPath) {
    ...
    //=========================================================
    vector<int>
        msgNumVec;  //$$$$$$$$$$$$$$$$$$$$ added for per-worker msg counting
    ...
    while (true) {
      ...
      if (wakeAll == 1)
        ...
        active_compute();
      message_buffer->combine();
      int my_msg_num = message_buffer->get_total_msg();  //$$$$$$$$$$$$$$$$$$$$
                                                         //added for per-worker
                                                         //msg counting
      msgNumVec.push_back(
          my_msg_num);  //$$$$$$$$$$$$$$$$$$$$ added for per-worker msg counting
